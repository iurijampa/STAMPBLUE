...
// OTIMIZADA: Agora usa JOIN e busca em uma única consulta para maior performance
export async function buscarAtividadesPorDepartamentoEmergencia(department: string) {
  ...
  try {
    ...
    // ETAPA 2: Buscar detalhes das atividades correspondentes - OTIMIZADO
    // Em vez de fazer uma consulta por atividade, fazemos uma única consulta para todas
    const activityIds = progressos.map(p => p.activityId);
    
    try {
      // Consulta otimizada usando IN para buscar todas as atividades de uma vez
      const atividades = await db
        .select()
        .from(activities)
        .where(inArray(activities.id, activityIds));
        
      console.log(`[EMERGENCIA] Recuperadas ${atividades.length} atividades com consulta otimizada`);
      
      // ETAPA 3: Ordenar por deadline (mais urgentes primeiro)
      const resultado = atividades.sort((a, b) => {
      ...
      console.error(`[EMERGENCIA] Erro na consulta otimizada, tentando método tradicional:`, err);
